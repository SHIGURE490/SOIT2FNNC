...
% lim(rad):joint1->[-2.7925,2.7925]
%joint2->[-0.7854,3.9270];
%joint3->[-3.9270,0.7854];
%joint4->[-1.9199,2.9671];
%joint5->[-1.7453,1.7453];
%joint6->[-4.6426,4.6426];
L(1) = Revolute('d', 0, ...   % link length (Dennavit-Hartenberg notation)
    'a', 0, ...               % link offset (Dennavit-Hartenberg notation)
    'alpha', pi/2, ...        % link twist (Dennavit-Hartenberg notation)
    'I', [0, 0.35*1.1, 0, 0, 0, 0], ... % inertia tensor of link with respect to center of mass I = [L_xx, L_yy, L_zz, L_xy, L_yz, L_xz]
...
L(2) = Revolute('d', 0, 'a', 0.4318, 'alpha', 0, ...
    'I', [0.13*1.1, 0.524*1.1, 0.539*1.1, 0, 0, 0], ...
    'r', [-0.3638*1.1, 0.006*1.1, 0.2275*1.1], ...
    'm', 17.4*1.1, ...
    'Jm', 200e-6, ...
    'G', 107.815, ...
    'B', .817e-3, ...
...
L(3) = Revolute('d', 0.15005, 'a', 0.0203, 'alpha', -pi/2,  ...
    'I', [0.066*1.1, 0.086*1.1, 0.0125*1.1, 0, 0, 0], ...
    'r', [-0.0203*1.1, -0.0141*1.1, 0.070*1.1], ...
    'm', 4.8*1.1, ...
    'Jm', 200e-6, ...
    'G', -53.7063, ...
    'B', 1.38e-3, ...
